...
{
    ComponentMapType::const_iterator componentIterator = ComponentMap.begin();
    const ComponentMapType::const_iterator componentEndIterator = ComponentMap.end();
    outputStream << "List of components: name and address" << std::endl;
    for (; componentIterator != componentEndIterator; ++componentIterator) {
        outputStream << "  Component: " << componentIterator->first << ", address: " << componentIterator->second << std::endl;
    }
}

void ToStreamDotClean(std::string & name)
{
    cmnStringReplaceAll(name, " ", "_");
    cmnStringReplaceAll(name, "-", "_");
    cmnStringReplaceAll(name, "+", "_plus_");
}
...
void mtsManagerLocal::ToStreamDot(std::ostream & outputStream) const
{
    ...
    outputStream << "/* Automatically generated by cisstMultiTask, mtsTaskManager::ToStreamDot.\n"
                 << "   Use Graphviz utility \"dot\" to generate a graph of components/devices interactions. */"
                 << std::endl;
    outputStream << "digraph mtsComponentManager {" << std::endl
                 << "graph[overlap=false];" << std::endl;
    // create all nodes for components
    ComponentMapType::const_iterator componentIterator = ComponentMap.begin();
    const ComponentMapType::const_iterator componentEndIterator = ComponentMap.end();
    for (; componentIterator != componentEndIterator; ++componentIterator) {
        outputStream << "subgraph cluster" << clusterNumber << "{" << std::endl;
        clusterNumber++;
        std::string cleanComponentName = componentIterator->first;
        ToStreamDotClean(cleanComponentName);
        // component node
        outputStream << cleanComponentName
                     << " [label=\"C: " << componentIterator->first << "\", color=grey, shape=box];" << std::endl;
        interfacesProvidedAvailable = componentIterator->second->GetNamesOfInterfacesProvided();
        for (stringIterator = interfacesProvidedAvailable.begin();
             stringIterator != interfacesProvidedAvailable.end();
             stringIterator++) {
            if (!mtsManagerComponentBase::IsNameOfInternalInterface(*stringIterator)
                && (stringIterator->substr(0, 10) != "StateTable")
                && (*stringIterator != "ExecOut")) {
                std::string fullInterfaceName = cleanComponentName + "__itf_provided__" + *stringIterator;
                ToStreamDotClean(fullInterfaceName);
                outputStream << fullInterfaceName
                             << " [label=\"P: " << *stringIterator << "\", color=lightgrey, shape=circle];" << std::endl;
                outputStream << fullInterfaceName
                             << "->" << cleanComponentName << ";" << std::endl;
            }
        }
        requiredInterfacesAvailable = componentIterator->second->GetNamesOfInterfacesRequired();
        for (stringIterator = requiredInterfacesAvailable.begin();
             stringIterator != requiredInterfacesAvailable.end();
             stringIterator++) {
            if (!mtsManagerComponentBase::IsNameOfInternalInterface(*stringIterator)
                && (*stringIterator != "ExecIn")) {
                std::string fullInterfaceName = cleanComponentName + "__itf_required__" + *stringIterator;
                ToStreamDotClean(fullInterfaceName);
                outputStream << fullInterfaceName
                             << " [label=\"R: " << *stringIterator << "\", color=lightgrey, shape=circle];" << std::endl;
                outputStream << cleanComponentName << "->"
                             << fullInterfaceName << ";" << std::endl;
                const mtsInterfaceProvided * connectedInterface = componentIterator->second->GetInterfaceProvidedFor(*stringIterator);
                if (connectedInterface) {
                    const mtsComponent * connectedComponent = connectedInterface->GetComponent();
                    std::string provided = connectedComponent->GetName() + "__itf_provided__" + *stringIterator;
                    ToStreamDotClean(provided);
                    outputStream << provided << "->" << fullInterfaceName << ";" << std::endl;
                }
            }
        }
